    ...
    def test_segmenter(self, model, test_files, test_ds):
        ...
        for x in range(len(test_files)):
            ...
            with torch.no_grad():
                ...
                # Compute ROI scores
                dice_ROI_unet = compute_meandice(binary_out, val_labels_convert_binary, include_background=False)[
                    0].cpu().numpy()
                ...
                cm = get_confusion_matrix(binary_out.contiguous(),
                                          ...
                                          include_background=False)
                sensitivity_ROI = compute_confusion_matrix_metric("sensitivity", cm)[0].cpu().numpy()
                specificity_ROI = compute_confusion_matrix_metric("specificity", cm)[0].cpu().numpy()
                precision_ROI = compute_confusion_matrix_metric("precision", cm)[0].cpu().numpy()
