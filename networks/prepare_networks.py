...
def get_nets(config):
    ...
    if config['input_type_class'] != 'img':
        ...
        lr_scheduler_seg = torch.optim.lr_scheduler.LambdaLR(
        ...
        (
            losses_train_init_seg, losses_valid_init_seg,
            best_metric_seg, iteration, epoch, binary_seg_weight
        ) = train_utils.try_load_ckpt(config['ckpt_dir'],
    ...
    if config['experiment_type'] == "classify" or config['experiment_type'] == "joint":
        ...
        classifier = train_utils.cuda(DenseNet121(
            spatial_dims=int(config['spatial_dims']),
        ...
        (
            losses_train_init_class, losses_valid_init_class,
            best_metric_class, iteration_class, epoch_class, _
        ) = train_utils.try_load_ckpt(config['ckpt_dir'],
